...
class SmartAngleLoss(nn.Module):
    def __init__(self, config):
        ...
        # --- 1. 读取权重 (确保读取 cosine_w) ---
        self.w_angle = loss_cfg.get('angle_w', 1.0)
        self.w_ibn = loss_cfg.get('ibn_w', 1.0)
        self.w_cosine = loss_cfg.get('cosine_w', 0.0) # [新增] 读取 Cosine 权重
        
        # --- 2. 读取温度系数 ---
        self.ibn_tau = loss_cfg.get('ibn_tau', 0.05)
        self.angle_tau = loss_cfg.get('angle_tau', 0.05)
        self.cosine_tau = loss_cfg.get('cosine_tau', 0.05) # [新增] 读取 Cosine 温度
        
        # --- 3. 难负样本挖掘参数 ---
        self.mining_alpha = loss_cfg.get('mining_alpha', 1.0)
        self.mining_lambda = loss_cfg.get('mining_lambda', 1.0)
    ...
    def forward(self, y_pred, y_true):
        ...
        # ==========================
        # Part A: Angle Optimization (复数角度优化)
        # ==========================
        ...
        loss_angle = (self.w_angle * dynamic_scale.unsqueeze(1) * (angle_diff ** 2) * labels_pos).mean()
        ...
        # 目标标签 (0->1, 1->0, 2->3...)
        target_indices = torch.arange(batch_size, device=y_pred.device)
        target_indices = (target_indices + 1) - 2 * (target_indices % 2)
        
        # ==========================
        # Part C: Standard Cosine Loss (保底模块 - 新增)
        # ==========================
        loss_cosine = 0.0
        if self.w_cosine > 0:
            # 标准对比学习，不加任何 tricky 的权重
            sim_matrix_cos = torch.matmul(z_norm, z_norm.T) / self.cosine_tau
            loss_cosine = F.cross_entropy(sim_matrix_cos, target_indices)

        # ==========================
        # Part D: IBN Loss + Mining (创新模块)
        # ==========================
        # 1. 基础相似度
        sim_matrix_ibn = torch.matmul(z_norm, z_norm.T) / self.ibn_tau
        
        # 2. 计算几何难样本权重
        hermitian_prod = torch.matmul(z, z.T.conj())
        angle_dist_matrix = torch.abs(torch.angle(hermitian_prod))
        
        # 创新点：加权
        hardness_weights = 1.0 + self.mining_alpha * torch.exp(-self.mining_lambda * angle_dist_matrix)
        
        # Mask (排除正样本和自身)
        mask = torch.eye(batch_size, device=y_pred.device)
        mask.scatter_(1, target_indices.unsqueeze(1), 1.0)
        final_weights = hardness_weights * (1 - mask) + 1.0 * mask
        
        # 注入权重
        weighted_logits = sim_matrix_ibn + torch.log(final_weights + 1e-9)
        loss_ibn = F.cross_entropy(weighted_logits, target_indices)
        
        # ==========================
        # Part E: 总 Loss
        # ==========================
        # 现在的 Loss 包含了三部分：角度 + 挖掘IBN + 基础Cosine
        total_loss = loss_angle + self.w_ibn * loss_ibn + self.w_cosine * loss_cosine
        return total_loss
...
if __name__ == '__main__':
    # 你的配置文件路径
    config_path = r"D:\WorkSpace\AnglE_yj\train\v4\config\train_config.json"
