...
class ComplexProjection(nn.Module):
    ...
    # def forward(self, x):# 将原始向量切分为实部和虚部基底 (维度 D/2)
    #     re_orig, im_orig = torch.chunk(x, 2, dim=1)
        
    #     # 实部保留残差：初始状态下 proj_re 为 0，所以 re = re_orig # (保留预训练模型的原始特征分布)
    #     re = re_orig + self.proj_re(re_orig)
        
    #     # 虚部去残差：初始状态下 proj_im 为 0，所以 im = 0 # (严格满足论文公式 h_im -> 0 的数学定义，仅由极性驱动)# 注意：这里我们依然使用 im_orig 作为输入来提取特征，但不把它加在外面
    #     im = self.proj_im(im_orig) 
...
class AngleLoss:
    ...
    def __call__(self,
        ...
        if self.dataset_format == 'A':
            ...
            # ====== [论文核心创新: ZRCP + AmpLoss] ======
            # <--- [修改] 删掉写死的 amp_w = 1.0，直接判断 self.amp_w
            if self.amp_w > 0:
                query = outputs[::2]
                positive_or_negative = outputs[1::2]

                query_re, query_im = torch.chunk(query, 2, dim=1)
                pos_re, pos_im = torch.chunk(positive_or_negative, 2, dim=1)

                query_amp = torch.sqrt(torch.sum(query_re**2 + query_im**2, dim=1))
                pos_amp = torch.sqrt(torch.sum(pos_re**2 + pos_im**2, dim=1))

                # 由于句子对来自于同一个Triplet的拆分，它们都在讨论同一个Aspect，模长必须一致！
                amp_loss = nn.MSELoss()(query_amp, pos_amp)

                # <--- [修改] 乘以来自配置文件的 self.amp_w
                loss += self.amp_w * amp_loss
            # =================================================================
            ...
            # 【不能删】必须保留这里的拆分！将打平的 3B 维度还原为 query, positive, negative 矩阵
            query = outputs[::3]
            ...
            # 为了复用二元计算函数，将三元组交叉组合为 (query, positive) 和 (query, negative)
            ...
            # ====== [论文核心创新: ZRCP AmpLoss (在三元组中的实现)] ======
            if self.amp_w > 0:
                # 1. 分离实部和虚部
                q_re, q_im = torch.chunk(query, 2, dim=1)
                p_re, p_im = torch.chunk(positive, 2, dim=1)
                n_re, n_im = torch.chunk(negative, 2, dim=1)

                # 2. 计算复数模长（振幅）
                q_amp = torch.sqrt(torch.sum(q_re**2 + q_im**2, dim=1))
                p_amp = torch.sqrt(torch.sum(p_re**2 + p_im**2, dim=1))
                n_amp = torch.sqrt(torch.sum(n_re**2 + n_im**2, dim=1))

                # 3. 约束正样本和负样本的振幅都向 query 对齐
                # (因为它们探讨的是同一个 Aspect，客观强度的基底应该一致)
                amp_loss_pos = nn.MSELoss()(q_amp, p_amp)
                amp_loss_neg = nn.MSELoss()(q_amp, n_amp)

                # 4. 将正负样本的振幅损失取平均后，乘以权重加入总损失
                loss += self.amp_w * (amp_loss_pos + amp_loss_neg) / 2.0
            # =========================================================
...
class AngleTrainerWrapper:
    def __init__(self, config_path=r"train_amplitude\train_v5\config\train_config.json"):
        ...
        try:
            ...
            if len(dataset) > 0:
                ...
                for i in range(min(3, len(dataset))):
                    ...
                    logger.info(f"    query: {sample.get('query', '')[:80]}...")
                    logger.info(f"    positive: {sample.get('positive', '')[:80]}...")
                    logger.info(f"    negative: {sample.get('negative', '')[:80]}...")
...
def main():
    ...
    try:
        # 可以指定不同的配置文件
        config_path = r"train_amplitude\train_v5\config\train_config.json"
