    ...
    def __call__(self,
        ...
        if self.dataset_format == 'A':
            ...
            if amp_w > 0:
                ...
                # 【新增测试代码：确认AmpLoss真的被算出来了】
                if random.random() < 0.05: # 5%的概率打印，防止刷屏                     
                    print(f"\n[Debug] 当前 AmpLoss 计算成功，值为: {amp_loss.item():.4f}")
...
class AngleTrainerWrapper:
    def __init__(self, config_path=r"train_learnable\v4\config\train_config.json"):
...
def main():
    ...
    try:
        # 可以指定不同的配置文件
        config_path = r"train_learnable\v4\config\train_config.json"
