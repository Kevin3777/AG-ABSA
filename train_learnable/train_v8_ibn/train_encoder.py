...
class AngleLoss:
    ...
    def __call__(self,
        ...
        if self.dataset_format == 'A':
            ...
            if self.ibn_w > 0:
                loss += self.ibn_w * in_batch_negative_loss(labels, outputs, self.ibn_tau, negative_weights=-1e12)
            if self.angle_w > 0:
...
class AngleTrainerWrapper:
    def __init__(self, config_path=r"train_learnable\train_v8_ibn\config\train_config.json"):
...
def main():
    ...
    try:
        # 可以指定不同的配置文件
        config_path = r"train_learnable\train_v8_ibn\config\train_config.json"
