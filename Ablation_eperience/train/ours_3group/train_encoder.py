...
class AngleLoss:
    ...
    def __call__(self,
        ...
        if self.dataset_format == 'A':
            ...
            # ====== [核心创新点 2: Amplitude Penalty (振幅惩罚)] ======
            # 我们强制要求 query 和 positive 的模长必须尽可能一致（因为它们属于同一个 Aspect）
            # 设置一个默认权重 amp_w，你可以把它暴露到 config 里，或者直接写死为 1.0
            amp_w = 0.0  
            if amp_w > 0:
                # 把向量劈成实部和虚部
                query_re, query_im = torch.chunk(query, 2, dim=1)
                pos_re, pos_im = torch.chunk(positive, 2, dim=1)
                
                # 计算复数模长 (Amplitude) -> sqrt(re^2 + im^2)
...
#     """从原始数据检测数据集格式"""
#     # 强制定义为C
#     print("[Debug-Format] 强制指定数据集格式为: C")
#     return 'C'
...
def load_raw_triplet_dataset(jsonl_file):
    import json
    ...
    print(f"[Data] 成功加载原生三元组数据 {len(data)} 条！(包含 query, positive, negative)")
...
class AngleTrainerWrapper:
    def __init__(self, config_path=r"Ablation_eperience\train\ours_3group\config\train_config.json"):
    ...
    def load_and_prepare_data(self):
        ...
        try:
            ...
            # 使用数据转换器加载和转换数据
            dataset = load_raw_triplet_dataset(input_file)
            ...
            if len(dataset) > 0:
                ...
                for i in range(min(3, len(dataset))):
                    sample = dataset[i]
                    logger.info(f"  样本 {i+1}:")
                    logger.info(f"    query: {str(sample.get('query', ''))[:80]}...")
                    logger.info(f"    positive: {str(sample.get('positive', ''))[:80]}...")
                    logger.info(f"    negative: {str(sample.get('negative', ''))[:80]}...")
...
def main():
    ...
    try:
        # 可以指定不同的配置文件
        config_path = r"Ablation_eperience\train\ours_3group\config\train_config.json"
