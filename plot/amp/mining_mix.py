...
def analyze_test_set_and_plot(csv_path=None):
    """
    分析测试集句子的振幅分布，并生成上下排列的复合图。
    
    Parameters
    ----------
    csv_path : str, optional
        CSV文件路径，默认为 "plot/amp/test_data_mining/test_sentence.csv"
    """
    ...
    # 创建2行1列的子图，总高度6英寸，宽度10英寸（可根据需要调整）
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(4, 6))
    plt.subplots_adjust(hspace=0.3)  # 调整上下子图间距，避免标题重叠
    ...
    # 保存为PDF和PNG两种格式
    output_dir = r'plot\amp'
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    plt.savefig(os.path.join(output_dir, 'test_set_amplitude_analysis.pdf'), format='pdf', bbox_inches='tight')
    plt.savefig(os.path.join(output_dir, 'test_set_amplitude_analysis.png'), format='png', bbox_inches='tight', dpi=300)
